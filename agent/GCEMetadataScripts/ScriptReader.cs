...
namespace GCEMetadataScripts
{
  ...
  {
    private const int RETRY_LIMIT = 5; // Give up after multiple failed attempts.
    ...
    /// </summary>
    /// <param name="suffix">The script type indicating the appropriate file extension.</param>
    ...
    /// </summary>
    /// <param name="gsurl">The URL to download.</param>
    /// <param name="fileName">The file where we should download the script.</param>
    /// <returns>A boolean representing whether the gsutil copy succeeded.</returns>
    private bool DownloadGSUrl(string gsurl, string fileName)
    {
      ...
      {
        Logger.Warning("Could not download {0} script using gsutil. Caught exception: [{1}].",
            this.scriptType, e.Message);
        return false;
      }
    }
    ...
    private bool DownloadUrl(string url, string fileName)
    {
      ...
      while (true)
      {
        ...
        {
          Logger.Warning("WebException downloading {0} script: [{1}], status: [{2}].",
              this.scriptType, e.Message, e.Status);
          if (attempt++ >= RETRY_LIMIT)
          {
            Logger.Warning("Could not download {0} script [{1}].", this.scriptType, url);
            return false;
          }
        }
        catch (Exception e)
        {
          Logger.Warning("Could not download {0} script [{1}]. Caught exception: [{2}].",
              this.scriptType, url, e.Message);
          return false;
        }
      }
    }
    ...
    /// <summary>
    /// Determines whether a requested URL is on Google Storage and downloads it.
    /// </summary>
    ...
    /// Saves the metadata script to a new file whose suffix matches the script type.
    /// </summary>
    /// <param name="suffix">The file type of the metadata script getting run.</param>
    /// <param name="scriptValue">The script itself, or the URL to the metadata script.</param>
    /// <returns>The MetadataScript object storing the script type and script filename.</returns>
    private MetadataScript FetchScript(string suffix, string scriptValue)
    {
      ...
      else
      {
        ...
        {
          Logger.Warning("Failed to write {0} script to file. Caught exception: [{1}].", this.scriptType, e.Message);
        }
    ...
    /// </summary>
    /// <param name="obj">The object whose property we want the value of.</param>
